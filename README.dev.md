# `cezerye` developer documentation
...
# install cezerye as an editable package
...
# recursively check import style for the cezerye module only
isort --recursive --check-only cezerye

# recursively check import style for the cezerye module only and show
# any proposed changes as a diff
isort --recursive --check-only --diff cezerye

# recursively fix import style for the cezerye module only
isort --recursive cezerye
```
...
cd $(mktemp -d --tmpdir cezerye.XXXXXX)

# fresh git clone ensures the release has the state of origin/main branch
git clone https://github.com/fdiblen/cezerye .
...
Visit
[https://test.pypi.org/project/cezerye](https://test.pypi.org/project/cezerye)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d --tmpdir cezerye-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple cezerye
```
